...
<script>
	function delCheck(no){
		if(confirm("삭제할까요?")){
			location.href='/myapp/franNoticeDelete?no='+no;
		}
	}
$(()=>{	
	$("#headerText").animate({
...
<div class="around">
	<div class="container">
	 	<div class="title_writedate">
			...
			<div><strong>등록일</strong> <span style="color:gray">${vo.writedate }</span></div> 
		</div>
...
<div class="btnList">
	...
	<button type="button" onclick="location.href='javascript:delCheck(${vo.no})'">삭제</button>
	<button type="button" onclick="location.href='/myapp/franNoticeEdit?no=${vo.no}'">수정</button>
	</c:if>
	<button type="button" onclick="location.href='/myapp/franNotice'">목록</button>
</div>	
